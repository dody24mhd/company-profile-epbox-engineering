$(function () {	
	...
	// Switch logo when QR mode is active
	const updateLogoForQR = () => {
		const isQRVisible = $('#qrcode').length && !$('#qrcode').hasClass('is-hidden');
		const $logo = $('#logo');
		if ($logo.length) {
			$logo.attr('src', isQRVisible ? 'logoblack.png' : 'logowhite.png');
		}
	};

	// Observe QR visibility changes
	const qrcodeNode = document.getElementById('qrcode');
	if (qrcodeNode) {
		const observer = new MutationObserver(updateLogoForQR);
		observer.observe(qrcodeNode, { attributes: true, attributeFilter: ['class'] });
	}

	// Also update on toggle link click
	$('#qrcode-link').on('click', function() {
		setTimeout(updateLogoForQR, 0);
	});
	...
	$.get($('#vcard-link').attr('href'), function( data ) {		
		...
		if ((vcard.fn && vcard.fn.length) || (vcard.nickname && vcard.nickname.length) || (vcard.org && vcard.org.length)) {
